...
- **下週到期需準備（UpcomingDue）**：bucket 為「未開始」且**到期日落在下週**（明天起 7 天）的任務。
...
- **上週**：(今天 - 6 天) 00:00 ～ 今天 23:59（共 7 天，以「今天」為結尾）。
- **下週**：明天 00:00 ～ (今天 + 7 天) 23:59（共 7 天，從「明天」開始）。

例：星期一執行 → 上週 = 上週二～今天星期一；下週 = 星期二～下週一。每次執行都以當下為基準，方便重複測試。依 `LOCAL_TIMEZONE`；需 Python 3.9+ 的 `zoneinfo`，若無則以 UTC 計算。

**排程窗格（schedule-nextweek）**：會排入「下週」區間。執行完成後會顯示「排入 YYYY-MM-DD ~ YYYY-MM-DD」供確認。
...
- 會將 **延續到下週** 與 **下週到期需準備** 的任務，依工作時段與行事曆空檔排入「下週」區間。
